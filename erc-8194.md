...
allows TMP (ERC-8195) to support human↔agent, agent↔agent, and human↔human task interactions
...
Forwarders SHOULD include `block.chainid` as the first field in the receipt hash encoding to
prevent receipt portability across chains (e.g., replay on a fork):
...
```
IPGTRForwarder interfaceId = 0xc47cd8cb
```

Computed as the XOR of:
- `isPGTRForwarder()` — `0xe1b0c012`
- `pgtrSender()` — `0x72e774dc`
- `isTrustedForwarder(address)` — `0x572b6c05`
...
The `IPGTRForwarder` interface ID is computed as the XOR of the 4-byte Keccak256 selectors of
all functions declared in the interface:

| Function | Keccak256 input | Selector |
|----------|----------------|----------|
| `isPGTRForwarder()` | `isPGTRForwarder()` | `0xe1b0c012` |
| `pgtrSender()` | `pgtrSender()` | `0x72e774dc` |
| `isTrustedForwarder(address)` | `isTrustedForwarder(address)` | `0x572b6c05` |

XOR: `0xe1b0c012 ^ 0x72e774dc ^ 0x572b6c05 = 0xc47cd8cb`

To compute in Solidity:
```solidity
bytes4 id = type(IPGTRForwarder).interfaceId; // 0xc47cd8cb
```
...
const xor = (a: string, b: string) =>
  '0x' + ((parseInt(a, 16) ^ parseInt(b, 16)) >>> 0).toString(16).padStart(8, '0');
const id = [
  toFunctionSelector('isPGTRForwarder()'),       // 0xe1b0c012
  toFunctionSelector('pgtrSender()'),             // 0x72e774dc
  toFunctionSelector('isTrustedForwarder(address)'), // 0x572b6c05
].reduce(xor); // 0xc47cd8cb
```
