...
solves **principal/signer separation**: the *principal* authorized via an off-chain payment
(typically X402 over HTTP) is bound to the on-chain action without needing to sign anything that
touches the destination contract. Two regimes follow from this:

- **Custodial / gateway flows** (X402 paid via a credit-card-to-stablecoin bridge, a WaaS
  provider, an exchange wallet, etc.): the principal genuinely holds no key. The custodian's
  wallet signs any ERC-3009 `transferWithAuthorization` needed for off-chain settlement; the
  principal authorizes only over HTTP. **This is full key abstraction.**
- **Direct-wallet flows** (the principal holds a wallet and self-signs an ERC-3009
  authorization over X402): the principal does sign a payment-layer message, but that signature
  is decoupled from the on-chain action. They get gas abstraction plus the property that the
  destination contract attributes the call to them rather than to the relayer, without needing
  to produce a per-action meta-transaction signature.

Either way, the **destination contract** never asks the principal for a signature; PGTR's value
is in the on-chain attribution semantics, not in eliminating cryptography from the entire stack.
...
3. Before forwarding, verify the payment receipt has not been consumed:
   ...
   bytes32 receiptHash = keccak256(abi.encode(
       block.chainid, payer, amount, nonce, expiry, target, selector
   ));
...
```
keccak256(abi.encode(chainId, payer, amount, nonce, expiry, target, selector))
```

Where:
- `chainId` — EVM chain ID (`block.chainid`); MUST be included to prevent cross-chain replay
...
2. **Receipt replay** — Forwarders MUST consume receipts atomically within the transaction that
   forwards the call. Receipts MUST include `chainId` in the hash to prevent cross-chain replay.
