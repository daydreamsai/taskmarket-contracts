...
The combined Diamond interface for callers that need to dispatch all facets through a single address is published at `src/interfaces/ITMPDiamond.sol` (promoted from an internal test helper in rev009). See `docs/specs/erc8195/rev009-accept-submissions-pinning.md` for the `acceptSubmissions` pinning extension and `--extend-expiry` validation fix.
