...
- **Bounty and Benchmark** modes use a **deferred-write** model. `submitWork` MUST emit
  `TaskSubmitted` but MUST NOT write `Task.deliverable` and MUST NOT change `Task.status`.
  Multiple workers MAY submit concurrently; the task remains `Open` throughout. Submission
  presence is tracked via the `TaskSubmitted` event log, not by a status transition.
  The deliverable is written into `Task.deliverable` only by `acceptSubmission` or
  `acceptSubmissions` at acceptance time, when the requester chooses the winning
  `(worker, deliverable)` pair.
...
`acceptSubmissions(taskId, workers[], shares[], deliverables[])` is the canonical N-winner
