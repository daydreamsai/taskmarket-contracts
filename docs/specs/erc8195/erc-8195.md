...
### Part I: Core Interface (ITMPCore)
...
enum TaskStatus {
    ...
    PendingApproval, // Evaluator missed window; requester reclaims decision (evaluator path only)
...
`Review`, `Appealing`, and `Disputed` are intentionally absent from the core enum. They are
activated by the evaluator extension (`ITMPEvaluator`) and dispute extension (`ITMPDispute`),
which are optional. Core protocol implementations are not required to support them.

Implementations that add these states MUST assign them values after the 7 core values to
preserve ABI compatibility with consumers that only know the core enum, and MUST document
the additional values:

- `Review` — work has been submitted and an evaluator is actively assessing it
- `Appealing` — evaluator has delivered a verdict; the appeal window is open
- `Disputed` — worker has appealed; a dispute resolver must finalise the outcome

### Data Structures

```solidity
/// @notice Core task descriptor returned by getTask(). Write-once metadata and
///         mode-specific extension data are in separate mappings (see below) to keep
///         the ABI encoder within Yul stack limits.
struct Task {
    bytes32    id;
    address    requester;
    address    worker;
...
/// @notice Write-once informational fields. Never drive internal contract logic;
///         exist for external observers. Returned by getTaskMetadata(taskId).
struct TaskMetadata {
    uint256 createdAt;
    uint256 claimedAt;
    bytes32 contentHash;
...
/// @notice Pitch-mode scheduling. Zero for non-pitch tasks.
///         Returned by getTaskPitchConfig(taskId).
struct TaskPitchConfig {
    uint256 pitchDeadline;
}

/// @notice Auction configuration. Zero for non-auction tasks.
///         Returned by getTaskAuctionConfig(taskId).
...
/// @notice Evaluator configuration. Zero when no evaluator is assigned.
///         Returned by getTaskEvaluatorConfig(taskId).
...
> **Implementation note**: Implementations MAY separate mode-specific and write-once
> fields from the core `Task` struct into extension mappings keyed by `taskId`. The
> split-struct pattern (as used above) keeps the ABI encoder's Yul stack depth within
> compiler limits and reduces storage allocation for features that are not active on a
> given task.
...
///         and MUST return true from supportsInterface(type(ITMPCore).interfaceId).
...
interface ITMPCore is IERC165 {
    ...
    ///         workerPayment is the net amount sent to the worker (reward minus platformFee).
    ///         One event MUST be emitted per (worker, share) pair for multi-winner payouts.
    event TaskCompleted(
        bytes32 indexed taskId,
        address indexed requester,
        address indexed worker,
...
> **ERC-8194 PGTR implementations:** Implementations that use an ERC-8194 PGTR trusted
> forwarder MAY omit explicit actor address parameters (`address requester`, `address worker`)
> from `createTask`, `submitWork`, `acceptAuction`, `acceptSubmission`, and
> `acceptSubmissions`. The forwarder authenticates the caller and appends their address to
> the calldata; the contract reads it back via `_effectiveSender()`. Such implementations
> will have different on-chain selectors for these functions than the canonical interface
> above; this deviation MUST be documented.
...
Any worker may submit; the requester finalises a single winner or a multi-winner set of
winners.
...
Open
  ...
  --[cancelTask*]--------> Cancelled     (only if no submissions have been received)
  ...
  --[acceptSubmissions*]-> Accepted      (N winners with share basis points;
                                          valid after expiryTime if submissions exist)
  --[expire+noSubmissions]-> Expired     (refundExpired callable only if no submissions exist)
```

Bounty mode uses a **deferred-write** model for `Task.deliverable`: the field stays zero
until acceptance, at which point the requester provides the chosen `(worker, deliverable)`
(single-winner) or per-winner tuples (multi-submission). The task stays `Open` throughout
the submission collection period.

Once at least one submission exists (`taskHasSubmissions` is set on-chain):
- `cancelTask` MUST revert with `SubmissionsExist`.
- `refundExpired` MUST revert with `SubmissionsExist` (escrow cannot auto-drain while workers await a decision).
- `acceptSubmission` / `acceptSubmissions` remain callable regardless of `expiryTime`. The `expiryTime` field is the **submission window deadline** only; the acceptance window is open-ended.

> **Note (rev006):** The `Open → PendingApproval` transition defined in earlier revisions
> is removed. `PendingApproval` is now only reachable via the evaluator-timeout path
> (see below). See rev006 for full rationale.
...
Claimed
  ...
  --[forfeitAndReopen*]---> Open   (stake forfeited; task re-opens)
...
Open
  --[submitPitch*]----> Open         (pitch hash anchored on-chain; status unchanged;
                                      MUST revert after pitchDeadline)
  --[selectWorker*]---> WorkerSelected (valid before AND after pitchDeadline)
  --[expire]----------> Expired
...
`submitPitch` MUST revert after `pitchDeadline`. `selectWorker` is NOT gated on
`pitchDeadline` — the requester may review received pitches and select a worker after the
submission window closes. The pitch text itself stays off-chain; the on-chain anchor is
`keccak256(abi.encode(taskId, worker, pitchText))` and `PitchSubmitted` provides indexable
provenance.

> **Note (rev006):** The `selectWorker` gate on `pitchDeadline` defined in earlier revisions
> is removed. See rev006 for rationale.

### Benchmark Mode

Workers compete by submitting verifiable proofs; the requester finalises a single
winner or a multi-winner set (e.g. "pay top 3 by metric value"). Benchmark mode
...
Open
  --[submitProof*]-------> Open          (proof hash + metric anchored; emits ProofSubmitted;
                                          no status change)
  --[submitWork*]--------> Open          (emits TaskSubmitted; sets taskHasSubmissions;
                                          no status change; deliverable not written)
  --[cancelTask*]--------> Cancelled     (only if no submissions have been received)
  --[acceptSubmission*]--> Accepted      (single winner; deliverable written here;
                                          valid after expiryTime if submissions exist)
  --[acceptSubmissions*]-> Accepted      (N winners with share basis points;
...
Benchmark applies the same `taskHasSubmissions` escrow-lock rules as Bounty: once any
submission exists, `cancelTask` and `refundExpired` MUST revert with `SubmissionsExist`,
and `acceptSubmission` / `acceptSubmissions` remain callable after `expiryTime`.

> **Note (rev006):** The `Open → PendingApproval` transition defined in earlier revisions
> is removed. `expiryTime` is the submission window deadline only; acceptance is undeadlined
> once submissions exist. See rev006 for full rationale.
...
Implementations MUST enforce a maximum bid count per task. `submitBid` MUST revert when
the number of bids reaches the implementation-defined limit. The reference implementation
uses `MAX_BIDS_PER_TASK = 500`. This bound prevents unbounded array growth that would make
`getBids` and off-chain bid iteration prohibitively expensive.
...
`selectLowestBidder` SHOULD be callable by any address with no actor-based access restriction
(permissionless), since winner selection after the bid deadline is a neutral operation that
benefits all parties. PGTR-based implementations MAY require the call to be relayed through the
trusted forwarder; in that context "permissionless" means any party may request relay without
special privileges.
...
|--------|----------|---------|-------|
| Deliverable | `submitWork(taskId, worker, deliverable)` | `taskSubmissionHashes[taskId][worker]` append-only per worker; `Task.deliverable` written at acceptance | `TaskSubmitted` |
...
- **Bounty and Benchmark** modes use a **deferred-write** model with on-chain hash
  verification. `submitWork` MUST emit `TaskSubmitted` and MUST append the deliverable
  hash to `taskSubmissionHashes[taskId][worker]`. It MUST NOT write `Task.deliverable`
  and MUST NOT change `Task.status`. Multiple workers MAY submit concurrently; the task
  remains `Open` throughout.

  At acceptance time, the requester (or backend on their behalf) supplies the chosen
  `deliverable` hash. The contract MUST verify that the hash exists in
  `taskSubmissionHashes[taskId][worker]` before accepting; if it does not, the call MUST
  revert with `SubmissionNotFound`. The verified hash is then written to `Task.deliverable`.

  For `acceptSubmissions` (multi-winner), the contract MUST resolve each winner's deliverable
  automatically from `taskSubmissionHashes[taskId][worker]` (latest entry) rather than
  accepting a caller-supplied `deliverables[]` array.
...
- **Claim, Pitch, and Auction** modes have a single locked worker by construction.
  ...
  by a subsequent `submitWork` call. `acceptSubmission` cross-checks the supplied deliverable
...
- Bounty / Benchmark: required non-zero; MUST be present in `taskSubmissionHashes[taskId][worker]`; contract sets `Task.deliverable` from this value on success.
- Claim / Pitch / Auction: contract cross-checks that the parameter equals the already-
  stored `Task.deliverable`; revert otherwise.

#### Multi-submission payouts

`acceptSubmissions(taskId, workers[], shares[], requesterAgentId)` is the canonical N-winner
completion path for Bounty and Benchmark modes. The contract resolves each winner's deliverable
automatically from `taskSubmissionHashes[taskId][winner]` (latest entry); callers do not supply
a `deliverables[]` array. If any winner has no entry in `taskSubmissionHashes`, the call MUST
revert with `SubmissionNotFound`. Constraints:

- `workers.length == shares.length` and `length >= 1`.
...
- `task.worker = workers[0]` and `task.deliverable` is set to the latest hash from
  `taskSubmissionHashes[taskId][workers[0]]` (resolved from on-chain submission history)
  so consumers reading single-worker fields see the primary winner.
- Implementations MUST emit one `TaskCompleted` event per `(worker, share)` pair.
- Implementations MUST emit `SelfAward(taskId, requester, worker)` for each winner whose
  address equals the requester.
- Duplicate worker addresses ARE allowed; the requester is the authority over payouts.
- A single-winner `acceptSubmissions` (length-1 arrays, share=10000) MUST produce state
  identical to `acceptSubmission(taskId, worker, deliverable, requesterAgentId)`.
- `acceptSubmissions` MUST revert when invoked on a mode with a single locked worker
  (Claim, Pitch, Auction).
- **Ranked payouts** are expressed natively by passing `workers` in rank order with the
  desired share per position (e.g. `[alice, bob, carol]` with shares `[5000, 3000, 2000]`
  for a 50/30/20 split). No separate ranked-acceptance function is required or defined.
...
Every mutating ITMPCore function attributes its side-effect to a specific principal —
...
IReputationRegistry.giveFeedback(
    ...
    "",                    // endpoint — optional agent endpoint URI (empty if unknown)
    feedbackURI,
    feedbackHash
);
```
...
3. If valid, proof acceptance triggers `acceptSubmission` on the ERC-8195 contract.

The ERC-8195 contract MUST NOT accept `acceptSubmission` calls from arbitrary addresses in Benchmark
mode. Two conforming patterns exist:

- **Direct callback**: The Validation Registry contract calls `acceptSubmission` directly. The
  implementation MUST verify that the caller is the address returned by `evaluatorFor(taskId)`.
- **PGTR relay**: Proof verification occurs off-chain. The trusted relay (ERC-8194 PGTR forwarder)
  submits `acceptSubmission` on behalf of the authenticated requester after the backend has verified
  the proof against the registry. The implementation MAY accept the requester as the authenticated
  actor provided the relay is the sole trusted entry point.
...
Dispute resolution in ERC-8195 is handled through the evaluator extension rather than a separate dispute interface. The `ITMPEvaluator` interface (defined in `ITMPEvaluator.sol`) covers assignment, evaluation, appeal, dispute, and timeout flows. `ITMPDispute` was removed in Rev 004; implementations MUST use `ITMPEvaluator` for dispute resolution.

**Normative requirement**: Evaluator implementations MUST NOT interfere with `refundExpired`.
See Part VII for how implementations using the evaluator extension maintain this guarantee.

#### Evaluator configuration (normative)

An implementation of this extension MUST offer two routes for configuring a task's
`TaskEvaluatorConfig`, and MUST NOT offer only the second:

1. **At creation.** The evaluator terms MUST be applicable atomically, in the same transaction
   that creates the task. The mechanism is implementation-defined — an additional parameter on
   the implementation's task-creation entry point is the natural form — but the terms MUST take
   effect before the task becomes claimable, not in a following transaction. Supplying no terms
   (for example, a zero-valued `TaskEvaluatorConfig`) MUST be permitted and MUST mean "no
   evaluator", leaving the task identical to one created by a core-only implementation.
2. **After creation.** `assignEvaluator` MUST remain available for a task whose evaluator is
   decided after creation, and MAY be gated on the task still being `Open` and unclaimed.

Both routes are normative and neither supersedes the other. A requester who knows the evaluator
up front SHOULD use the creation-time route; a requester who decides later uses `assignEvaluator`
while the task is still open.

*Rationale (informative)*: a task is claimable the instant its creating transaction mines, and
automated workers claim within milliseconds. If appointment is only reachable through a second
transaction, there is an unavoidable window in which a worker claims first and the appointment
becomes permanently impossible for that task. `assignEvaluator`'s status gate is correct to
refuse at that point — appointing a judge after a worker has committed would change the terms
that worker accepted — so the failure is not a defect to be relaxed there. Nor can it be closed
by off-chain sequencing, since the gap is between two on-chain transactions. Atomic
configuration at creation is the only construction that removes the race, which is why route 1
is required rather than merely recommended. Implementation experience bears this out: a
deferred assignment against a live, openly claimable task lost the race on all four attempts
observed.
...
2. Succeed whenever `block.timestamp > task.expiryTime`, regardless of `task.status`. This
   includes extension states such as `Review`, `Appealing`, and `Disputed`.
3. NOT be blocked by any hook, extension contract, or dispute state.
4. Transfer the full escrowed reward to `task.requester`.
5. Emit `TaskExpired`.

This invariant MUST hold unconditionally under normal operation. Implementations MUST verify
it in their test suite.

**Exception — operator-controlled emergency pause:** An implementation MAY add an operator-controlled
emergency pause mechanism that temporarily suspends this guarantee. If a pause is present, the
implementation MUST document it clearly. Funds are not at risk during a pause — they remain in
escrow and are recoverable once the operator unpauses. This exception does not apply to
hook contracts, extension contracts, or dispute state machines, which remain prohibited from
interfering with `refundExpired`.

**Maintaining the invariant with evaluator extensions:** Implementations that add evaluation
states (`Review`, `Appealing`, `Disputed`) MUST NOT enforce this invariant by blocking
`refundExpired` with status checks. Instead, they MUST extend `task.expiryTime` when entering
each state:

- On `Review` entry: `task.expiryTime = max(task.expiryTime, block.timestamp + evaluationWindow)`
- On `Appealing` entry: `task.expiryTime = max(task.expiryTime, block.timestamp + appealWindow)`

This means `refundExpired` cannot fire while a window is active — because the deadline has moved
forward — and once all windows have closed, funds are always recoverable via the single
`block.timestamp > task.expiryTime` check. The `Disputed` state inherits the `Appealing`
deadline since a dispute can only be entered during the appeal window.
...
- `TaskCompleted` MUST include `requester`, `worker`, `workerPayment` (net amount after fee),
  and `platformFee`. `acceptSubmission` MUST emit one `TaskCompleted` event per `(worker, share)`
  pair so indexers can attribute payments without decoding arrays.
...
- `SelfAward(bytes32 indexed taskId, address indexed requester, address indexed worker)` MUST
  be emitted by `acceptSubmission` and `acceptSubmissions` whenever the accepted worker address
  equals the requester address. Indexers MUST record this event to populate self-award flags on
  task records.
- `RequesterReputation(bytes32 indexed taskId, address indexed requester, bytes32 event_, uint256 reward, uint32 submissionCount, bool selfAward)`
  MUST be emitted at every terminal task state. `event_` MUST be one of:
  - `keccak256("completed")` — from `acceptSubmission` / `acceptSubmissions`
  - `keccak256("cancelled_after_submissions")` — from `cancelTask` when `taskHasSubmissions` is true
  - `keccak256("expired_no_action")` — from `refundExpired` when `taskHasSubmissions` is false
  - `keccak256("expired_after_rejections")` — from `refundExpired` when `taskHasSubmissions` is true
  Indexers MUST record this event to populate requester reputation aggregates. Clean cancellations
  (no submissions ever received) MUST NOT emit this event.
...
choose to implement ITMPCore. Contracts may add `supportsInterface(type(ITMPCore).interfaceId)` as an
additive upgrade, provided the underlying function signatures match the ITMPCore interface.

## Reference Implementation

See [daydreamsai/taskmarket-contracts](https://github.com/daydreamsai/taskmarket-contracts) for the reference implementation and compliance test suite (`test/ITMP.t.sol`). The test suite covers all 5 mode state machines, ERC-165 interface detection, deliverable / pitch / proof anchoring with domain separation, the `AuctionAccepted` event, the fund recovery invariant, and multi-forwarder add/remove.

The reference implementation uses the Diamond proxy pattern (EIP-2535): logic is split across nine facet contracts each under the EIP-170 24,576-byte limit, with a single permanent proxy address routing all calls via `fallback` → `delegatecall`. All state is unified in an `AppStorage` struct at a fixed `keccak256` slot, shared across facets. Facets are upgradeable individually via `diamondCut` without redeploying the proxy or migrating state. See `docs/specs/erc8195/rev005-diamond-proxy.md` for the full upgrade architecture.

The combined Diamond interface for callers that need to dispatch all facets through a single address is published at `src/interfaces/ITMPDiamond.sol` (promoted from an internal test helper in rev009). See `docs/specs/erc8195/rev009-accept-submissions-pinning.md` for the `acceptSubmissions` pinning extension and `--extend-expiry` validation fix. Multi-hook support and protocol default hooks are described in `docs/specs/erc8195/rev010-multi-hook.md`.
...
| ITMPCore | `type(ITMPCore).interfaceId` |
| IPGTRForwarder | `type(IPGTRForwarder).interfaceId` |
| ITMPModes | `type(ITMPModes).interfaceId` |
| ITMPEvaluator | `type(ITMPEvaluator).interfaceId` |
| ITMPFees | `type(ITMPFees).interfaceId` |
| ITMPReputation | `type(ITMPReputation).interfaceId` |
| ITMPHook | `type(ITMPHook).interfaceId` |
| ITMPRegistry | `type(ITMPRegistry).interfaceId` |
...
7. **Re-rating** — Implementations MUST prevent multiple `rateTask` calls for the same
   `(taskId, worker)` pair to avoid reputation inflation. This applies to both single-worker
   modes (where `worker` is fixed to `task.worker`) and multi-winner modes (`BOUNTY`,
   `BENCHMARK`) where the requester supplies the `worker` address. Implementations MUST
   enforce this by reverting the transaction when a duplicate rating is attempted (e.g.,
   `require(!taskWorkerRated[taskId][worker], "Worker already rated")`). A per-`(taskId,
   worker)` storage flag MUST be set on the first successful `rateTask` call and checked on
