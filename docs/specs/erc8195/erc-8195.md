...
```
Open
  --[submitWork*]--------> PendingApproval  (first submission; emits TaskSubmitted;
                                             no deliverable write)
  --[expire]-------------> Expired

PendingApproval
  --[submitWork*]--------> PendingApproval  (subsequent submissions; emits TaskSubmitted;
...
`(worker, deliverable)` (single-winner) or per-winner tuples (multi-submission). This allows N
workers to concurrently submit competing artifacts via `submitWork`; the on-chain audit
trail is the per-worker `TaskSubmitted` event stream. The `Open → PendingApproval`
transition on first submission signals that at least one submission exists without
closing the task to further submissions.
...
PendingApproval
  --[submitProof*]-------> PendingApproval  (subsequent proofs; emits ProofSubmitted)
  --[submitWork*]--------> PendingApproval  (subsequent submissions; emits TaskSubmitted;
                                             no deliverable write; status unchanged)
  --[acceptSubmission*]--> Completed        (single winner; deliverable written here)
  --[acceptSubmissions*]-> Completed        (N winners with share basis points)
...
- **Bounty and Benchmark** modes use a **deferred-write** model. `submitWork` MUST emit
  `TaskSubmitted` but MUST NOT write `Task.deliverable`. Multiple workers MAY submit
  concurrently. The first `submitWork` call MUST transition the task from `Open` to
  `PendingApproval`; subsequent calls MUST leave status as `PendingApproval`. The
  deliverable is written into `Task.deliverable` only by `acceptSubmission` or
