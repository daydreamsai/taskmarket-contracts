...
market contract without implementation-specific knowledge. TMP is built on PGTR (ERC-8194),
...
// Auction subtype selectors — passed as auctionSubtype in createTask
...
struct TaskInfo {
    ...
    bytes4    auctionSubtype; // Auction subtype selector (bytes4(0) for non-auction tasks)
    address   lowestBidder;  // Running lowest bidder (english/reverse_english subtypes; zero otherwise)
    uint256   lowestBidPrice;// Running lowest bid price (zero if no bids yet)
}

/// @notice Cumulative statistics for a worker address.
/// @dev avgRating = ratedTasks > 0 ? (totalStars * 100) / ratedTasks : 0
struct WorkerStats {
    uint256 completedTasks;   // Tasks accepted (payment released to this worker)
    uint256 ratedTasks;       // Number of tasks that received a rating
    uint256 totalStars;       // Raw rating sum; divide by ratedTasks for the average
}
```
...
interface ITMP is IERC165 {
    ...
    /// @dev    Called by a trusted PGTR forwarder. The requester is read via
    ///         IPGTRForwarder(msg.sender).pgtrSender() (Pattern A, ERC-8194 normative).
    ...
    /// @param auctionSubtype  Auction subtype selector (TMP_AUCTION_DUTCH etc.; bytes4(0) for non-auction tasks).
    ...
    /// @dev    Called by a trusted PGTR forwarder. The worker is read via
    ///         IPGTRForwarder(msg.sender).pgtrSender() (Pattern A, ERC-8194 normative).
    ...
    /// @dev    Called by a trusted PGTR forwarder. The requester is read via
    ///         IPGTRForwarder(msg.sender).pgtrSender() and MUST equal the task requester.
    ///         Emits TaskAccepted. MUST NOT be callable after TaskStatus.Expired.
    /// @param taskId    Task identifier.
    ...
    ///         The requester is read via IPGTRForwarder(msg.sender).pgtrSender().
...
### PGTR Destination Pattern

TMP destination contracts MUST implement the PGTR destination requirements from ERC-8194.
The normative pattern reads the authenticated actor via `IPGTRForwarder(msg.sender).pgtrSender()`:
...
// Functions take NO explicit requester/worker param; actor is read via _effectiveSender().
```

The reference implementation (`TaskMarket.sol`) uses this pattern. Functions that operate on
behalf of the calling actor do not accept an explicit address parameter — the forwarder sets
`pgtrSender` atomically for the duration of the call, and `_effectiveSender()` reads it.
...
Workers compete on a verifiable metric. Submission and acceptance follow the same flow as Bounty
mode. The distinction is semantic: off-chain tooling is expected to verify the metric before the
requester calls `acceptSubmission`.
...
ERC-8004 Validation Registry integration (automated on-chain metric verification) is a planned
extension. When implemented, `evaluatorFor(taskId)` will return the registry address and
`acceptSubmission` will be gated to registry-only calls in Benchmark mode. See the `ITMPMode`
optional interface.

### Auction Mode

Supports four subtypes: `dutch`, `english`, `reverse_dutch`, `reverse_english`.
Clock-based subtypes (dutch, reverse_dutch) use `acceptAuction` for atomic acceptance at the
current clock price. Sealed-bid subtypes (english, reverse_english) record bids then select
the winner after the bid deadline.

The auction subtype is stored on-chain in `TaskInfo.auctionSubtype` as a `bytes4` selector
(e.g. `TMP_AUCTION_DUTCH`). Subtype-specific rules (clock price computation, bid ordering,
re-bid policy) are enforced by the off-chain backend. The contract provides the primitives
(`submitBid`, `selectLowestBidder`, `acceptAuction`) and the backend
applies the correct flow for the chosen subtype.

```
Open
  --[submitBid*]-------> Open        (sealed bid recorded; status unchanged)
  --[acceptAuction*]---> Claimed     (clock-based subtypes: dutch, reverse_dutch)
  --[selectLowestBidder]--> Claimed  (sealed-bid subtypes: english, reverse_english; after bidDeadline)
  --[expire]-----------> Expired

Claimed (winner locked)
  --[submitWork*]----> Claimed       (deliverable hash recorded; status unchanged)
  --[acceptSubmission]--> Accepted
  --[expire]----------> Expired (winner auto-paid in refundExpired)
```

Note: `selectLowestBidder` MUST be O(1). Conforming implementations MUST maintain a running
minimum (`lowestBidder`, `lowestBidPrice`) in `submitBid` so that winner selection reads
directly from task storage without iterating the bid array.
...
### Validation Registry (Planned Extension)

Automated Benchmark mode verification via the ERC-8004 Validation Registry is a planned
extension and is not part of the current normative spec. When implemented, the flow will be:

1. The forwarder submits the worker's proof to the ERC-8004 Validation Registry off-chain.
2. The Validation Registry evaluates the proof against the task's `metricTarget`.
3. If valid, the Registry calls `acceptSubmission` on the TMP contract.

When this extension is active, `evaluatorFor(taskId)` MUST return the registry address and
the TMP contract SHOULD gate `acceptSubmission` in Benchmark mode to registry-only calls.

Until this extension is deployed, Benchmark mode `acceptSubmission` is callable by the
requester (identical to Bounty mode).
...
interface ITMPMode is IERC165 {
    ...
    ///         Bounty/Claim/Pitch/Auction: returns task.requester
    ///         Benchmark: returns ERC-8004 Validation Registry (when registry integration
    ///                    is active); returns task.requester otherwise.
    /// @dev    This function is OPTIONAL in the current reference implementation.
    ///         Benchmark mode Validation Registry gating is a planned extension.
...
4. Transfer the full escrowed reward to `task.requester` — **except** for Auction tasks in
   `Claimed` status (winner selected): in this case the implementation MUST auto-pay the worker
   at the winning bid price, refund any excess reward to the requester, and transition status to
   `Accepted`. This preserves the winner's earned payment while recovering unspent escrow.
5. Emit `TaskExpired` (for normal expiry) or `TaskAccepted` (for Auction auto-pay on expiry).
...
- `TaskAccepted` MUST include worker address and reward amount.
...
5. **Auction gas** — `selectLowestBidder` MUST be O(1) (see auction state machine note above).
   The running minimum (`lowestBidder`, `lowestBidPrice`) is maintained incrementally in
   `submitBid`, so winner selection costs a single SLOAD regardless of bid count.
