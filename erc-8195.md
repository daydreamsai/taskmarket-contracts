...
### Breaking changes from the previous Draft revision

This revision adjusts ITMP signatures and lifts the PGTR dependency. Implementations
following an earlier Draft MUST update:

- `acceptSubmission(bytes32 taskId, address requester, address worker)` gains a
  fourth `bytes32 deliverable` parameter.
- `rateTask(bytes32 taskId, uint8 rating, …)` gains a second `address worker`
  parameter so multiple winners can be rated separately.
- `acceptRanked(bytes32 taskId, address[] workers, uint16[] shares, bytes32[] deliverables)`
  is a new required function. Implementations that do not support modes with multiple
  concurrent submissions MAY revert unconditionally; the function MUST still exist on
  the contract for ABI compatibility.
- `submitWork` in Bounty and Benchmark modes is now an emit-only operation. Implementations
  MUST NOT write `Task.deliverable` or transition status in these modes; the deferred
  write happens at acceptance time.
- `isTrustedForwarder(address) external view returns (bool)` is no longer a required
  ITMP view function. Implementations using ERC-8194 PGTR will still expose it for
  ERC-8194 compliance, but ITMP itself is settlement-agnostic.

Pre-mainnet implementations are encouraged to upgrade in lockstep. There is no backwards-
compatible migration; the ABI changes are intentional.
